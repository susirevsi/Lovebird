<?php
	...
	$jml_data = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "select * from penyakit"));
	...
	if(isset($_GET['hal'])){
			...
			$s = mysqli_query($GLOBALS["___mysqli_ston"], "select * from penyakit order by kode_penyakit desc limit ".(($hal - 1) * $jml_per_halaman).", $jml_per_halaman");
		
	}else{
		$s = mysqli_query($GLOBALS["___mysqli_ston"], "select * from penyakit order by kode_penyakit desc limit 0, $jml_per_halaman")
